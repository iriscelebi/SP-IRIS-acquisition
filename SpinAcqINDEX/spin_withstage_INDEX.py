...
def __go_right(_=None):
	...
	if __X_POS + __XY_STEP < __XY_POS_MAX:
		...
		stageXY.__moveRelative(1, int(__XY_STEP*1e3))
		__update_pos_x()
		
	else:
		print('Step exceeds limits, moving to max position')	
		stageXY.__moveRelative(1, int((__XY_POS_MAX - __X_POS)*1e3))
...
def __go_right_plus(_=None):
	...
	if __X_POS + __XY_STEP_PLUS < __XY_POS_MAX:
		...
		stageXY.__moveRelative(1, int(__XY_STEP_PLUS*1e3))
		__update_pos_x()
...
def __go_left(_=None):
	...
	if __X_POS - __XY_STEP > 0:
		...
		stageXY.__moveRelative(1, int((- __X_POS)*1e3))
		__X_POS = 0
		__update_pos_x()
...
def __go_up(_=None):
	...
	if __Y_POS + __XY_STEP < __XY_POS_MAX:
		...
		stageXY.__moveRelative(2, int(__XY_STEP*1e3))
		__update_pos_y()
		
	else:
		print('Step exceeds limits, moving to max position')	
		stageXY.__moveRelative(2, int((__XY_POS_MAX - __Y_POS)*1e3))
...
def __go_up_plus(_=None):
	...
	if __Y_POS + __XY_STEP_PLUS < __XY_POS_MAX:
		...
		stageXY.__moveRelative(2, int(__XY_STEP_PLUS*1e3))
		__update_pos_y()
...
def __go_down(_=None):
	...
	if __Y_POS - __XY_STEP > 0:
		...
		print('Step exceeds limits, moving to min position')	
		stageXY.__moveRelative(2, int((- __Y_POS)*1e3))
		__Y_POS = 0
		__update_pos_y()
...
def __go_defocus_up(_=None):
	...
	__Z_POS = min( __Z_POS + __Z_STEP,20)
	stageZ.__setVolt(__Z_POS/20.0*75)
	__update_pos_z()

def __go_defocus_up_plus(_=None):
	global __Z_POS
	
	__Z_POS = min(  __Z_POS + __Z_STEP_PLUS,20)
	stageZ.__setVolt(__Z_POS/20.0*75)
	__update_pos_z()
